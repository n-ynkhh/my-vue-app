...
def process_directory(directory_path):
    ...
    for filename in os.listdir(directory_path):
        if filename.endswith('.pdf'):
            ...
            single_record = {"ファイル名": filename, "タイプ": "単体"}
            consolidated_record = {"ファイル名": filename, "タイプ": "連結"}
            
            # 質問ごとにデータを割り当てる
            for key, value in extracted_data.items():
                if "単体決算" in key:
                    single_record[key.replace("単体決算_", "")] = value
                elif "連結決算" in key:
                    consolidated_record[key.replace("連結決算_", "")] = value
            
            # 単体データが存在する場合にのみ追加
            if any(single_record.get(q.replace("単体決算_", ""), "") != "" for q in extracted_data):
                data.append(single_record)
            
            # 連結データが存在する場合にのみ追加
            if any(consolidated_record.get(q.replace("連結決算_", ""), "") != "" for q in extracted_data):
                data.append(consolidated_record)
    return data
