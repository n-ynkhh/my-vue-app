...
while true; do
    ...
    echo $DATA | jq -c '.[]' | while read -r record; do
        ...
        echo "MERGE INTO $SNOWFLAKE_TABLE AS t USING (SELECT '$id' AS sf_id, '$name' AS name, '$custom_field' AS custom_field FROM DUAL) AS s ON t.sf_id = s.sf_id WHEN MATCHED THEN UPDATE SET t.name = s.name, t.custom_field = s.custom_field WHEN NOT MATCHED THEN INSERT (sf_id, name, custom_field) VALUES (s.sf_id, s.name, s.custom_field);" >> "$FILE_NAME"
    done
