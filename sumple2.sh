...
while : ; do
    ...
    echo "$projects" | while IFS= read -r project; do
        IFS=',' read -r project_id project_name namespace_path <<< "$(echo $project | jq -r '.[]')"
        mr_page=1
        while : ; do
            ...
            echo "$merge_requests" | jq -r --arg project_name "$project_name" --arg namespace_path "$namespace_path" '.[] | [$namespace_path, $project_name, (.id|tostring), .title, .created_at, .state, .author.username, .merged_at, .source_branch, .web_url] | @csv' >> $OUTPUT_FILE
