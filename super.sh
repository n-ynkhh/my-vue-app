...
PAGE=1
while : ; do
    projects=$(curl -s "https://gitlab.com/api/v4/projects?private_token=$PRIVATE_TOKEN&page=$PAGE&per_page=$PER_PAGE" | jq -c '.[] | select(.forked_from_project == null)')
...
# SnowSQLでSQLコマンドを実行
echo "$SQL_COMMANDS" | snowsql --username YOUR_USERNAME --dbname YOUR_DBNAME --schemaname YOUR_SCHEMANAME --warehousename YOUR_WAREHOUSENAME -o exit_on_error=true
