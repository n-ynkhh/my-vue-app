echo "$PROJECTS" | jq -c '.[] | select((.forked_from_project | length) == 0 or .forked_from_project == null)' | while read i; do
    # プロジェクトの処理...
done
...
echo "$PROJECTS" | jq -c '.[] | select((.forked_from_project | length) == 0 or .forked_from_project == null)' | while read i; do
    ...
    # ブランチの取得とページネーションの処理...
    BRANCHES=$(curl --header "Private-Token: $TOKEN" "$GITLAB_URL/api/v4/projects/$PROJECT_ID/repository/branches")
    echo "$BRANCHES" | jq -r '.[].name' | while read BRANCH; do
        # コミットの取得とページネーションの処理...
        COMMITS=$(curl --header "Private-Token: $TOKEN" "$GITLAB_URL/api/v4/projects/$PROJECT_ID/repository/commits?ref_name=$BRANCH&since=$SINCE_DATE&until=$UNTIL_DATE")
