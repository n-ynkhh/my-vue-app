...
curl --header "Private-Token: $TOKEN" "$GITLAB_URL/api/v4/projects?per_page=100" | jq -c '.[]' | while read i; do
    ...
    # プロジェクトの全ブランチを取得
    curl --header "Private-Token: $TOKEN" "$GITLAB_URL/api/v4/projects/$PROJECT_ID/repository/branches?per_page=100" | jq -c '.[]' | while read b; do
        BRANCH_NAME=$(echo $b | jq -r '.name')

        # ブランチのコミットを取得
        curl --header "Private-Token: $TOKEN" "$GITLAB_URL/api/v4/projects/$PROJECT_ID/repository/commits?ref_name=$BRANCH_NAME&since=$START_DATE&until=$END_DATE&per_page=100" | jq -c '.[]' | while read j; do
            ...
            # INSERT文の値部分を出力
            echo -n "(\"$GROUP_NAME\", \"$PROJECT_NAME\", \"$BRANCH_NAME\", \"$AUTHOR_NAME\", \"$COMMIT_DATE\", \"$COMMIT_HASH\", \"$COMMIT_MESSAGE\", $ADDED_LINES, $DELETED_LINES, $IS_MERGE_COMMIT)" >> "$OUTPUT_FILE"
        done
    done
done
