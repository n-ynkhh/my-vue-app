            ...
            echo "$pipelines" | jq -r '.[] | .id' | while read pipeline_id; do
                ...
                while : ; do
                    ...
                    if [ "$BATCH_START" -eq 1 ]; then
                        ...
                        # バッチ内の新しいレコードの前にコンマを追加
                        echo "," >> "$SQL_FILE"
                    fi

                    # レコードをSQLファイルに追加
                    echo "$jobs" | jq -r --arg project_id "$project_id" --arg project_name "$project_name" '.[] | "(\(.id), \(.pipeline.id), \($project_id), \($project_name), \(.ref), \(.name), \(.status), \(.stage), \((.tag_list | join(","))), \(.web_url), \(.created_at), \(.started_at), \(.finished_at), \(.erased_at // "null"), \(.duration // "null"), \(.queued_duration // "null"), \(.user.id), \(.user.name), \((if .runner then .runner.description else "N/A" end))"' | sed '$!s/$/,/' | sed '$ s/,$/;/' >> "$SQL_FILE"

                    LINE_COUNTER=$((LINE_COUNTER + $job_count))

                    # バッチの最大サイズに達した場合、次のバッチを開始
